...
<head>
    <meta charset="UTF-8">
    <title>Editar Servicio - Hotel Rizzo</title>
    ...
    <main class="main-content">
        <div class="form-container">
            <div class="form-header">
                ...
                <p class="form-subtitle">Modifique los datos del servicio <%= servicio.getNombre() %></p>
            </div>
            
            <form action="servicio_editar" method="post" class="servicio-form">
                <input type="hidden" name="id" value="<%= servicio.getIdServicio() %>">
                
                <div class="form-group">
                    <label for="nombre">Nombre del Servicio</label>
                    <input type="text" id="nombre" name="nombre" required 
                           value="<%= servicio.getNombre() %>" placeholder="Nombre del servicio">
                </div>
                
                <div class="form-group">
                    <label for="descripcion">Descripción</label>
                    <textarea id="descripcion" name="descripcion" rows="4"
                              placeholder="Descripción detallada del servicio"><%= servicio.getDescripcion() %></textarea>
                </div>
                
                <div class="form-group">
                    <label for="precio">Precio Unitario (S/)</label>
                    <input type="number" id="precio" name="precio" step="0.01" min="0" required 
                           value="<%= servicio.getPrecioUnitario() %>" placeholder="0.00">
                </div>
                
                <div class="form-actions">
                    <a href="servicio_detalle?id=<%= servicio.getIdServicio() %>" class="btn btn-secondary">
